...
def xtime(a):
    """Multiplica per 0x02 en GF(2^8)"""
    result = (a << 1) & 0xFF  # Desplaçament a l'esquerra i mantenim 8 bits
    if a & 0x80:  # Si el bit més significatiu era 1
        result ^= 0x1B  # Reducció amb el polinomi irreductible
    return result

def gmul(a, b):
    """
    Multiplicació simplificada en GF(2^8) per a MixColumns.
    Només gestiona els casos de a = 0x01, 0x02, 0x03.
    """
    if a == 0x01:
        return b
    if a == 0x02:
        return xtime(b)
    if a == 0x03:
        return xtime(b) ^ b  # 0x03 * b = (0x02 * b) ⊕ b
    raise ValueError("gmul només suporta multiplicacions per 0x01, 0x02, 0x03 a MixColumns")
